...
- last 30 days: the consumo page opens with the big numbers of the month (tokens, what the model wrote, calls, the biggest day) and one bar per day. it reads the same local claude code transcripts as the insights, so it only counts this machine.
- long chats: a card draws only its last 200 messages, so a chat with thousands of them stays as fast as a new one. `mostrar anteriores` at the top brings older ones back, 200 at a time.
